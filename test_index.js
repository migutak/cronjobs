...
// var task = cron.schedule('0 6 * * *', () => {});
  ...
  /*(async function() {
    try {
      ...
      result = await connection.execute("update demandsdue set status = 'self cure' where status = 'pending' and accnumber not in (select accnumber from loans_stage)");
        ...
        try {
          ...
          result = await connection.execute("select * from autoletters where active = 'true'");
           for (i=0; i<result.rows.length; i++) {
              ...
              const sqlInsert = "select accnumber,custnumber,client_name, section, oustbalance, instamount totalarrears, daysinarr, nvl(addressline1,0) address, postcode postalcode, telnumber, emailaddress, colofficer, branchcode, rrocode, arocode, '"+letterid+"' demand from tqall where substr(accnumber,3,3) = '" + memogrp + "' and daysinarr = " + daysinarr;
